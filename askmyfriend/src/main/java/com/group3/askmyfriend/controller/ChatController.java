...
public class ChatController {
    ...
    //채팅 메세지 신고
    @PostMapping("/api/report/message")
    @ResponseBody
    public ResponseEntity<Void> reportChatMessage(@RequestBody ChatReportRequestDto dto) {
        try {
            Long currentUserId = getCurrentUserId();
            chatService.reportChatMessage(dto, currentUserId);
            return ResponseEntity.ok().build();
        } catch (Exception e) {
